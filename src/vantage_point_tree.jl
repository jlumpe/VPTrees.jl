...
# Default for skip argument
alwaysfalse(args...; kw...) = false
...
"""
    find(vptree::VPTree{InputType, MetricReturnType}, query::InputType, radius::MetricReturnType[, skip::Function])::Vector{Int}
...
```
"""
function find(vptree::VPTree{InputType, MetricReturnType}, query::InputType, radius::MetricReturnType, skip=alwaysfalse)::Vector{Int} where {InputType, MetricReturnType}
    if vptree.threaded
...
function _find(vantage_point, query, radius, results, metric, skip)
    ...
    if distance <= radius && !skip(vantage_point.index)
...
```
"""
function find_nearest(vptree::VPTree{InputType, MetricReturnType}, query::InputType, n_neighbors::Int, skip=alwaysfalse)::Vector{Int} where {InputType, MetricReturnType}
